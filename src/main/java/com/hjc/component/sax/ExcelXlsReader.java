...
/**
 * @author by hjc
 * @Classname ExcelXlsReader
 * @Description Excel2003读取工具类
 * @Date 2019/7/9 21:36
 */
@Slf4j
public class ExcelXlsReader implements HSSFListener {
    ...
    public int process(InputStream inputStream) {
        try {
            ...
            log.error("excel parse exception is {}", ex);
        }
    ...
    public void processRecord(Record record) {
        ...
        switch (record.getSid()) {
            ...
            case BlankRecord.sid: //单元格为空白
                ...
                thisStr = "";
                cellList.put(thisColumn, thisStr);
                break;
        ...
        if (record instanceof LastCellOfRowDummyRecord) {
            ...
            if (flag&&curRow!=0) { //该行不为空行且该行不是第一行，发送（第一行为列名，不需要）
                excelReaderUtil.sendRows(filePath, sheetName, sheetIndex, curRow + 1, cellList,null,null,null); //每行结束时，调用sendRows()方法
                totalRows++;
            }
            //清空容器
            cellList.clear();
